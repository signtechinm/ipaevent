...
if (path === 'admin/mailer/test' && request.method === 'POST') {
        ...
        if (path === 'admin/event-participation' && request.method === 'GET') {
            ...
            const participationRows = await sql`SELECT registration_number, event_name, participated FROM participant_event_participation WHERE participated = TRUE`;
            const students = rows.flatMap((row) => row.registration_mode === 'group' ? normalizeGroupMembers(row.group_members).map((m, i) => ({ registrationNumber: m.registrationNumber || groupMemberRegistrationNumber(row.registration_number, i), parentRegistrationNumber: row.registration_number, name: m.name || `Student ${i + 1}`, events: [...(m.competitions || []), ...(m.presentationType === 'Poster Presentation' ? ['Poster Presentation'] : [])] })) : [{ registrationNumber: row.registration_number, name: row.participant_name || row.group_coordinator_name || '', events: [...(row.student_competitions || []), ...(row.presentation_type === 'Poster Presentation' ? ['Poster Presentation'] : [])] }]);
            const selectedByNumber = participationRows.reduce((result, p) => ({ ...result, [p.registration_number]: { ...(result[p.registration_number] || {}), [p.event_name]: true } }), {});
            return send(response, 200, { events: [...programs.map((p) => p.name), 'Poster Presentation'], students: students.map((s) => ({ ...s, events: s.events.filter(Boolean), selected: selectedByNumber[s.registrationNumber] || selectedByNumber[s.parentRegistrationNumber] || {} })) });
        }
